...
def main():
    parser = argparse.ArgumentParser(
        description="Converts SNPs in VCF format into an alignment for phylogenetic analysis")
            ...
            for line in vcf_chunk:
                if not line.startswith("#") and line.strip(
                    ...
                    if (len(broken[9:]) - ''.join(broken[9:]).count("./.")) >= min_samples_locus:
                        ...
                        if len(broken[3]) == 1 and (len(broken[4]) - broken[4].count(",")) == (
                            broken[4].count(",") + 1):
                            ...
                            if fasta or nexus or not phylipdisable:
                                ...
                                site_tmp = ''.join([(amb[(nuc[broken[i][0]], nuc[broken[i][2]])]) for i in
                                                    range(9, index_last_sample)])
                            ...
                            if nexusbin:
                                ...
                                if len(broken[4]) == 1:
                                    ...
                                    binsite_tmp = ''.join(
                                        [(gen_bin[broken[i][0:3]]) for i in range(9, index_last_sample)])

                                    # Write entire row to temporary file
                                    temporalbin.write(binsite_tmp + "\n")

                        elif len(broken[3]) == 1 and broken[4].startswith("<INS:ME"):
                            # Is a MEI
    ...
    if nexus:
        ...
        header_nex = "#NEXUS\n\nBEGIN DATA;\n\tDIMENSIONS NTAX=" + str(len(sample_names)) + " NCHAR=" + str(
        ...
        if s != idx_outgroup:
            if fasta or nexus or not phylipdisable:
                with open(outfile + ".tmp") as tmp_seq:
                    ...
                    # Print current progress
                    print "Sample " + str(s + 1) + " of " + str(len(sample_names)) + ", " + sample_names[
                        s] + ", added to the nucleotide matrix(ces)."
